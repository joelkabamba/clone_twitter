        ...
        <div class="py-6">
            <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                <form @submit.prevent="tweetStore">
                    <textarea v-model="content" placeholder="Que se passe-t-il ?" class="rounded-lg border border-gray-200 w-full p-2 font-semibold 
                    resize-one focus:outline-none">
                    </textarea>
                    <span class="my-5 text-red-500" v-if="$page.errors.content">
                        {{ $page.errors.content }}
                    </span>
                    <div class="flex items-center space-x-4 justify-end mt-3">
                        <p :class="{'text-red-500': remainingChars < 0}" class="text-sm text-gray-400 font-thin">
                            {{ remainingChars }} caracteres restants
                        </p>
